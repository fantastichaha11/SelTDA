# Filter vs. Post-training, Rubric-8, and Judge-only Experiment Design
...
This experiment package tests three distinct claims without rerunning conditions that already exist:
...
`PT-4-J` is the only new post-training condition that removes the external penalties. `PT-4-JP` and `PT-8-JP` both retain the existing penalty weights.
...
Across both datasets, the design therefore requires six new end-to-end conditions: `Filter-4`, `PT-4-J`, and `PT-8-JP` for PathVQA, plus the same three conditions for VizWiz. Existing `SelTDA` and `PT-4-JP` results are reused.
...
\[
N_{\mathrm{syn}} = 40{,}000 - N_{\mathrm{real}},
\]

where `N_real` is the number of real examples actually admitted by the downstream student loader. Every condition must satisfy:

\[
N_{\mathrm{real}} + N_{\mathrm{syn}} = 40{,}000.
\]

The run manifest must record `N_real`, `N_syn`, and the final loader-observed total. A condition is invalid if it merely sets a truncation maximum of 40,000 but does not demonstrate that the loader admitted exactly 40,000 examples.

All methods use the same real examples. Only the source and selection of the `N_syn` synthetic examples change.
...
2. Generate until the eligible raw pool contains at least `2 * N_syn` pseudo-QA examples. If the final generation batch overshoots, retain the first `2 * N_syn` eligible examples by stable generation index so the scored pool is exactly 2x.
...
7. Retain the first `N_syn` examples and combine them with the fixed real subset.

The filtered output must report:

- raw generation attempts;
- eligible pool size, which must equal `2 * N_syn`;
- retained synthetic count, which must equal `N_syn`;
...
After post-training, each teacher generates exactly `N_syn` valid synthetic examples using the same downstream generation settings. Each resulting student uses the same initialization, real subset, optimizer, batch size, epoch count, data order seed, and evaluation procedure.

### 6.1 Existing penalty condition (`JP`)

Both `PT-4-JP` and `PT-8-JP` use the existing penalty weights:
...
- final real and synthetic counts;
...
The filter receives a 2x candidate pool by design. This is not an equal-generation-compute comparison. The final student data budget is equal, while generation cost, judge calls, GPU-hours, and discarded candidates are reported as efficiency outcomes.
...
For the final synthetic datasets from `Filter-4`, `PT-4-J`, `PT-4-JP`, and `PT-8-JP`, report:
...
5. Run diagnostics, paired bootstrap, and efficiency aggregation.

Every expensive stage must support resume without overwriting a completed checkpoint or dataset. Each condition writes to a distinct output directory. A run stops and is invalidated if it produces non-finite losses, fails to reach the specified update count, cannot prove the final 40,000-example loader count, or silently falls back to a different rubric or teacher checkpoint.
...
1. all six new conditions finish or a failure is explicitly documented;
2. every downstream loader admits exactly 40,000 examples;
3. `Filter-4` ranks an eligible global pool of exactly `2 * N_syn` without a per-image quota;
4. `PT-4-J` has zero external penalties and uses only the Rubric-4 score as reward;
5. `PT-4-JP` and `PT-8-JP` use identical nonzero penalty settings;
6. the effective four- and eight-criterion rubric texts are archived;
7. validation/test metrics, paired bootstrap intervals, data diagnostics, and compute accounting are produced for both datasets;
8. claims in the thesis follow the interpretation rules in Section 10.
