#!/bin/bash
# Minimal blip conda env for GCP (avoids clip==1.0 PyPI failure in environment.yaml).
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck source=conda_helpers.sh
source "${SCRIPT_DIR}/conda_helpers.sh"

init_conda

BLIP_PIP=(
  omegaconf==2.2.2 hydra-core==1.2.0 attrs==22.1.0 cattrs==22.1.0
  numpy==1.23.1 opencv-python==4.6.0.66 timm==0.4.12 fairscale==0.4.4
  transformers==4.21.3 huggingface_hub==0.13.4 open_clip_torch==2.20.0
  sentence-transformers==2.2.2 tqdm pillow gdown einops
)

if blip_ready; then
  echo "[skip] blip env ready"
  exit 0
fi

if conda env list | grep -q '^blip '; then
  echo "Repairing incomplete blip env..."
  run_blip pip install --upgrade pip wheel setuptools
  run_blip pip install torch==2.0.1 torchvision==0.15.2 \
    --index-url https://download.pytorch.org/whl/cu118
  run_blip pip install "${BLIP_PIP[@]}"
  run_blip pip install git+https://github.com/openai/CLIP.git
  echo "blip env repaired"
  exit 0
fi

conda create -n blip python=3.8 -y
run_blip pip install --upgrade pip wheel setuptools
run_blip pip install torch==2.0.1 torchvision==0.15.2 \
  --index-url https://download.pytorch.org/whl/cu118
run_blip pip install "${BLIP_PIP[@]}"
run_blip pip install git+https://github.com/openai/CLIP.git

echo "blip env ready"
