...
TRAIN_JSON="datasets/pathvqa/train.json"
VAL_JSON="datasets/pathvqa/val.json"
TEST_JSON="datasets/pathvqa/test.json"
SEED=42
TARGET_TOTAL=40000
...
run_rubric_probe() {
  ...
  require_file cache/pathvqa_teacher_weights/checkpoint_04.pth
  run python scripts/diagnose_rubrics.py prepare \
    --validation-annotations "${VAL_JSON}" \
    --image-root "${IMAGE_ROOT}" \
    --teacher-checkpoint cache/pathvqa_teacher_weights/checkpoint_04.pth \
  ...
  generate_pool cache/pathvqa_teacher_weights/checkpoint_04.pth 8 "${RAW_DIR}/probe_generation_images.json"
  ...
  run python scripts/diagnose_rubrics.py score \
    ...
    --image-root "${IMAGE_ROOT}" \
    --rubric4-config configs/prometheus_judge_pathvqa.yaml \
    --rubric8-config configs/prometheus_judge_pathvqa_rubric8.yaml \
    --selected-model-path "${SELECTED_PROMETHEUS_JUDGE:-cache/prometheus-vision-7b-v1.0}" \
    --output-dir "${EXP_ROOT}" \
...
run_filter4() {
  ...
  generate_pool cache/pathvqa_teacher_weights/checkpoint_04.pth "${qpi}" "${RAW_DIR}/train_unique_images.json"
  run python scripts/score_rank_prometheus_pool.py --config configs/global_filter_pathvqa_rubric4.yaml
  train_student synthetic_filter4_40k "pathvqa-filter4-40k"
  evaluate_student
}
...
run_posttrain_40k() {
  ...
  write_build_config "datasets/pathvqa/${synthetic_name}.json" "${RAW_DIR}/generated_qa.json"
  ...
  train_student "${synthetic_name}" "pathvqa-${CONDITION}-40k"
  evaluate_student
}
...
run_60k() {
  prepare_dirs
  preflight_common
  PT4_JP_TEACHER_CKPT="${PT4_JP_TEACHER_CKPT:-outputs/grpo_teacher/pathvqa_prometheus_base7b_200x3epochs_strongpenalty_restart_20260710/checkpoint_step_000600.pth}"
  PT4_JP_SYNTHETIC_40K="${PT4_JP_SYNTHETIC_40K:?set PT4_JP_SYNTHETIC_40K to the exact existing PathVQA Full-40K synthetic JSON}"
  ...
  write_build_config "datasets/pathvqa/synthetic_pt4_jp_60k.json" "${RAW_DIR}/extra_generated_qa.json" "${PT4_JP_SYNTHETIC_40K}"
  run python scripts/build_student_synthetic.py --config "${DATA_DIR}/build_config.yaml"
  run python scripts/build_student_synthetic.py --verify-manifest "${DATA_DIR}/manifest.json"
  train_student synthetic_pt4_jp_60k "pathvqa-pt4-jp-60k"
  evaluate_student
}
...
case "${CONDITION}" in
  ...
  filter4) run_filter4 ;;
  pt4_j) run_posttrain_40k configs/grpo_teacher_pathvqa_rubric4_judge_only.yaml "${EXP_ROOT}/teacher/checkpoint_step_000600.pth" synthetic_pt4_j_40k ;;
  pt8_jp) run_posttrain_40k configs/grpo_teacher_pathvqa_rubric8_penalties.yaml "${EXP_ROOT}/teacher/checkpoint_step_000600.pth" synthetic_pt8_jp_40k ;;
  pt4_jp_60k) run_60k ;;
